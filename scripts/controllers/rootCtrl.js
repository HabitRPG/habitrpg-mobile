...
habitrpg.controller('RootCtrl',
  ['$scope', '$rootScope', '$location', '$ionicNavBarDelegate', 'User', '$state', '$stateParams', '$window', '$ionicPlatform', 'Groups', 'ApiUrlService', '$http',
  function ($scope, $rootScope, $location, $ionicNavBarDelegate, User, $state, $stateParams, $window, $ionicPlatform, Groups, ApiUrlService, $http) {

    $rootScope.User = User;
    $rootScope.user = User.user;
    $rootScope.set = User.set;
    $rootScope.settings = User.settings;
    $rootScope.settings.auth.apiEndpoint = ApiUrlService.get();
    $rootScope.postData = function(endpoint, data){
      return $http.post(ApiUrlService.get()+endpoint, data);
    };
