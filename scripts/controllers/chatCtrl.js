...
habitrpg.controller('ChatCtrl',
  ['$rootScope','$scope', 'User', '$resource', 'ApiUrl', '$state', '$http',
    function($rootScope, $scope, User, $resource, ApiUrl, $state, $http) {
      if ($state.params.gid) {
        $state.current.data.gid = $state.params.gid;
        $scope.guildChat = true;
      }
      $scope._message = {text:undefined};
      $scope.Chat = $resource(ApiUrl.get() + "/api/v2/groups/:gid/chat",
        {gid:'@_id'}, {
      ...
      $scope.postChat = function(_message) {
        ...
        startSyncing();
        $http.post(ApiUrl.get() + '/api/v2/groups/' + $state.current.data.gid + '/chat', {}, {params:{message:_message.text}})
          .success(function(){
            $scope.query();
